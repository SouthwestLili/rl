...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.random() < epsilon:
        ...
        best_command_index = np.argmax(q_values)

        action_index, object_index = index2tuple(best_command_index)
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    Returns:
        None
    """
    command_index = tuple2index(action_index, object_index)

    # Current Q(s, c, theta)
    current_q = (theta @ current_state_vector)[command_index]
    ...
    # Gradient update only for the current command's weight vector
    theta[command_index] += (
        ALPHA * (target - current_q) * current_state_vector
    )
...
def run_episode(for_training):
    ...
    while not terminal:
        # Convert current text state to feature vector
