...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.random() < epsilon:
        # Exploration: randomly select a command
        action_index = np.random.randint(NUM_ACTIONS)
        object_index = np.random.randint(NUM_OBJECTS)

    else:
        # Exploitation: select the command with the largest Q-value
        q_values = q_func[state_1, state_2]

        flat_index = np.argmax(q_values)

        action_index, object_index = np.unravel_index(
            flat_index, q_values.shape
        )
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    Returns:
        None
    """
    current_q = q_func[current_state_1, current_state_2,
    ...
    q_func[current_state_1, current_state_2,
           action_index, object_index] = (
        (1 - ALPHA) * current_q + ALPHA * target
    )
...
def run_episode(for_training):
    ...
    while not terminal:
        ...
        # Convert current text state to indices
        ...
        # Convert next text state to indices
