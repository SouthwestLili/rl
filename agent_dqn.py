...
def epsilon_greedy(state_vector, epsilon):
    ...
    """
    # Exploration: randomly choose action and object
    if np.random.random() < epsilon:
        action_index = np.random.randint(NUM_ACTIONS)
        object_index = np.random.randint(NUM_OBJECTS)

    # Exploitation: choose action/object with highest predicted Q-value
    else:
        with torch.no_grad():
...
def deep_q_learning(current_state_vector, action_index, object_index, reward,
    ...
    # Q(s, c)
    q_value_cur_state = 1 / 2 * (
        q_values_action_cur[action_index] +
    ...
    # Squared TD error
    loss = 1 / 2 * (target - q_value_cur_state) ** 2
...
def run_episode(for_training):
    ...
    while not terminal:
        # Current state -> BOW vector -> tensor
